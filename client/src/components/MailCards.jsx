...
export default function MailCards({ isDark, emails = [], setSelectedEmail, isTrashMode = false, selectedEmails = [], onSelectEmail, onEmailUpdate }) {
  ...
  return (
    <div
      className={`flex flex-col gap-4 sm:gap-8 py-4 sm:py-8 px-4 sm:px-8 ${
        isDark ? "bg-transparent" : "bg-[#F3F6FA]"
      }`}
    >
      {emails.map((email) => (
        <div
          key={email._id}
          className={`rounded-xl sm:rounded-3xl flex flex-col sm:flex-row p-3 sm:p-4 sm:px-10 cursor-pointer transition-all duration-200 shadow-xl sm:shadow-2xl hover:scale-[1.01] border-2 ${
            isDark
              ? "bg-gradient-to-br from-[#232326]/90 via-[#18181b]/90 to-[#232326]/80 border-[#232326]"
              : "bg-white border-[#e5e7eb] "
          } ${
            selectedEmails.includes(email._id) ? "ring-2 ring-blue-500" : ""
          }`}
          onClick={() =>
            isTrashMode ? onSelectEmail(email._id) : setSelectedEmail(email)
          }
        >
          <div className={`flex items-center gap-3 sm:gap-4 flex-1 min-w-0`}>
            ...
            <img
              ...
              className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center font-bold relative flex-shrink-0 text-lg sm:text-xl object-cover`}
            />
                ...
                <div className="flex gap-2">
                  ...
                  {!email.to?.[0]?.readAt &&
                    !location.pathname.includes("/sent") && (
                      <span className="bg-blue-500 text-white px-2 py-0.5 rounded-full text-xs flex items-center justify-center whitespace-nowrap">
                        New
                      </span>
                    )}
                </div>
